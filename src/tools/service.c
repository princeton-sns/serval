...
static int service_parse_args(int argc, char **argv, void **result)
{
	...
	if (argc < 3)
        ...
        while (argc) {
                ...
                else if (strcmp("source", argv[0]) == 0) {

                        // my_buff[18];

                        if (argc < 2) {
                                fprintf(stderr, "No source IP address given\n");
                                return -1;
                        }

                        // printf("Source IP address: %s\n", argv[1]);

                        ret = name_to_inet_addr(argv[1], &args.srcaddr);

                        //printf("Source IP address: %s\n", inet_ntop(AF_INET, &args.srcaddr, my_buff, 18));
