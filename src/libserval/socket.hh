...
private:
  ...
  int query_serval_listen(int backlog, const sv_srvid_t& local_obj_id, 
			 Cli &cli, sv_err_t &err);
  ...
  int query_serval_send(bool nb, const void *buffer, size_t length, int flags,
		       Cli &cli, sv_err_t &err);
  int query_serval_sendto(const sv_srvid_t& dst_obj_id, uint32_t ipaddr,
  ...
  int query_serval_migrate(Cli &cli, sv_err_t &err);
