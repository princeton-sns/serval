...
static struct dest *dest_create(const void *dst, int dstlen, const void* dest_out, uint32_t weight,
        gfp_t alloc) {
    struct dest *de;
...
static struct dest_set *dset_create(uint16_t flags, uint32_t priority, gfp_t alloc) {
...
static struct dest *__service_entry_get_dev(struct service_entry *se, const char *ifname) {
    struct dest *de;
    ...
    list_for_each_entry(dset, &se->dest_set, ds) {
        list_for_each_entry(de, &dset->dest_list, lh) {
            if(!is_sock_dest(de) && de->dest_out.dev && strcmp(de->dest_out.dev->name, ifname) == 0) {
                return de;
            }
        }
    }
...
static struct dest * __service_entry_get_dest(struct service_entry *se, const void *dst,
        int dstlen, struct dest_set** dset_p) {
    struct dest *de = NULL;
    struct dest_set* dset = NULL;

    list_for_each_entry(dset, &se->dest_set, ds) {
        list_for_each_entry(de, &dset->dest_list, lh) {
            if((is_sock_dest(de) && dstlen == 0) || (!is_sock_dest(de) && memcmp(de->dst, dst,
                    dstlen) == 0)) {
                if(dset_p)
                    *dset_p = dset;
                return de;
...
/* 
 ...
 function.
 */
struct net_device *service_entry_get_dev(struct service_entry *se, const char *ifname) {
    struct dest *de = NULL;
...
static void service_entry_insert_dset(struct service_entry *se, struct dest_set* dset) {
...
static struct dest_set* __service_entry_get_dset(struct service_entry *se, uint32_t priority) {
    struct dest_set* pos = NULL;
...
static int __service_entry_add_dest(struct service_entry *se, uint16_t flags, uint32_t priority,
        uint32_t weight, const void *dst, int dstlen, const void *dest_out, gfp_t alloc) {
    struct dest_set* dset = NULL;
...
int service_entry_add_dest(struct service_entry *se, uint16_t flags, uint32_t priority,
        uint32_t weight, const void *dst, int dstlen, const void *dest_out, gfp_t alloc) {
    int ret = 0;

    write_lock(&se->destlock);
    ret = __service_entry_add_dest(se, flags, priority, weight, dst, dstlen, dest_out, GFP_ATOMIC);
    write_unlock(&se->destlock);
...
static int __service_entry_modify_dest(struct service_entry *se, uint16_t flags, uint32_t priority,
        uint32_t weight, const void *dst, int dstlen, const void *dest_out, gfp_t alloc) {
    struct dest_set* dset = NULL;
    struct dest *de = __service_entry_get_dest(se, dst, dstlen, &dset);
...
int service_entry_modify_dest(struct service_entry *se, uint16_t flags, uint32_t priority,
        uint32_t weight, const void *dst, int dstlen, const void *dest_out) {
    int ret = 0;

    write_lock(&se->destlock);
    ret = __service_entry_modify_dest(se, flags, priority, weight, dst, dstlen, dest_out,
            GFP_ATOMIC);
    write_unlock(&se->destlock);
...
static void __service_entry_inc_dest_stats(struct service_entry *se, const void* dst, int dstlen, int packets, int bytes) {
    struct dest_set* dset = NULL;
    struct dest *de = __service_entry_get_dest(se, dst, dstlen, &dset);
...
int __service_entry_remove_dest_by_dev(struct service_entry *se, const char *ifname) {
    struct dest *de;
...
int __service_entry_remove_dest(struct service_entry *se, const void *dst, int dstlen,
...
void service_resolution_iter_init(struct service_resolution_iter* iter, struct service_entry *se,
        int all) {
    /* lock the se, take the top priority entry and determine the extent of iteration*/
    struct dest_set* dset;
    int sumweight = 0;
    ...
    if(all || (dset->flags & SVSF_MULTICAST)) {
        ...
        sample = (uint32_t) ((float) sample / 0xFFFFFFFF * dset->normalizer);
#else

        sample = (uint32_t) ((float) rand() / RAND_MAX * dset->normalizer);
#endif
...
#define PREFIX_BUFLEN (sizeof(struct service_id)*2+4)
    ...
    list_for_each_entry(dset, &se->dest_set, ds) {
        list_for_each_entry(de, &dset->dest_list, lh) {
            len += snprintf(buf + len, buflen - len, "%-64s %-6u %-6u %-6u %-6u ", prefix, bits,
                    dset->flags, dset->priority, de->weight);

            if(is_sock_dest(de) && de->dest_out.sk) {
                len += snprintf(buf + len, buflen - len, " %-20s\n", de->dest_out.sk ? "sock"
                        : "NULL");
            } else if(!is_sock_dest(de) && de->dest_out.dev) {
...
static struct service_entry *service_table_find(struct service_table *tbl,
        struct service_id *srvid, int prefix) {
...
static struct sock* service_table_find_sock(struct service_table *tbl, struct service_id *srvid,
        int prefix) {
    struct service_entry *se = NULL;
    struct bst_node *n;
    struct sock* sk = NULL;
...
static int service_table_modify(struct service_table *tbl, struct service_id *srvid,
        uint16_t prefix_bits, uint16_t flags, uint32_t priority, uint32_t weight, const void *dst,
    ...
    if(n && bst_node_prefix_bits(n) >= prefix_bits) {
        if(dst || dstlen == 0) {
            ret = __service_entry_modify_dest(get_service(n), flags, priority, weight, dst, dstlen,
                    dest_out, GFP_ATOMIC);
        }
        goto out;
    }
    ...
    out: read_unlock_bh(&tbl->lock);

    return ret;
}
int service_modify(struct service_id *srvid, uint16_t prefix_bits, uint16_t flags,
        uint32_t priority, uint32_t weight, const void *dst, int dstlen, const void* dest_out) {
    return service_table_modify(&srvtable, srvid, prefix_bits, flags, priority, weight, dst,
            dstlen, dest_out);
}

static int service_table_add(struct service_table *tbl, struct service_id *srvid,
        uint16_t prefix_bits, uint16_t flags, uint32_t priority, uint32_t weight, const void *dst,
        int dstlen, const void *dest_out, gfp_t alloc) {
    struct service_entry *se;
    struct bst_node *n;
    int ret = 0;
    ...
    if(n && bst_node_prefix_bits(n) >= prefix_bits) {
        if(dst || dstlen == 0) {
            ret = __service_entry_add_dest(get_service(n), flags, priority, weight, dst, dstlen,
                    dest_out, GFP_ATOMIC);
    ...
    if(dest_out) {
        ret = __service_entry_add_dest(se, flags, priority, weight, dst, dstlen, dest_out,
                GFP_ATOMIC);
    ...
    se->node = bst_insert_prefix(&tbl->tree, &tbl->srv_ops, se, srvid, prefix_bits, GFP_ATOMIC);
...
int service_add(struct service_id *srvid, uint16_t prefix_bits, uint16_t flags, uint32_t priority,
        uint32_t weight, const void *dst, int dstlen, const void* dest_out, gfp_t alloc) {
    return service_table_add(&srvtable, srvid, prefix_bits, flags, priority, weight, dst, dstlen,
            dest_out, alloc);
}
...
static void service_table_del_dest(struct service_table *tbl, struct service_id *srvid,
        uint16_t prefix_bits, const void *dst, int dstlen, struct dest_stats* stats) {
    struct bst_node *n;
    int ret = 0;
    //if (!dst || dstlen == 0)
    ...
    if(n) {
        ...
        ret = __service_entry_remove_dest(get_service(n), dst, dstlen, stats);
        if(ret > 0) {
            tbl->instances--;
        }
...
void service_del_dest(struct service_id *srvid, uint16_t prefix_bits, const void *dst, int dstlen,
        struct dest_stats* stats) {
    return service_table_del_dest(&srvtable, srvid, prefix_bits, dst, dstlen, stats);
}
...
static int service_table_del_dev_all(struct service_table *tbl, const char *devname) {
    int ret = 0;
...
static int service_table_del_dest_all(struct service_table *tbl, const void *dst, int dstlen) {
    int ret = 0;
    struct {
        const void *d_dst;
        int d_len;
