                                               ...
                                               struct sk_buff *skb)
{
        struct sal_hdr *sh;
        struct serval_sock *ssk = serval_sk(sk);
        unsigned short hdr_len = 0, pad_len;
        ...
        if (pad_len) {
                /* We must add some padding for the total SAL header
                 * to be divisible by word size (4 bytes). */
                hdr_len += serval_sal_add_pad_ext(sk, skb, 4 - pad_len);
        }
