...
struct sv_local_resolver {
    ...
    task_mutex_t peer_mutex;
    GHashTable *peer_table;

    task_rwlock_t res_lock;
...
static void local_set_capabilities(service_resolver * resolver,
                                   uint32_t capabilities);

static int local_peer_discovered(service_resolver * resolver,
                                 service_resolver * peer, uint16_t type);
static int local_peer_disappeared(service_resolver * resolver,
                                  service_resolver * peer, uint16_t type);

static int local_register_services(service_resolver * resolver,
                                   service_resolver * peer,
                                   struct service_desc *services,
                                   size_t num_svc,
                                   struct net_addr *address, uint32_t ttl);
static int local_register_services_async(service_resolver *resolver,
                                         service_resolver *peer,
                                         struct service_desc *services,
                                         size_t num_svc,
                                         struct net_addr *address,
...
static int local_unregister_services(service_resolver *resolver,
                                     service_resolver *peer,
                                     struct service_desc *services,
                                     size_t num_svc, struct net_addr *address);
static int local_unregister_services_async(service_resolver * resolver,
                                           service_resolver * peer,
                                           struct service_desc *services,
                                           size_t num_svc,
                                           struct net_addr *address,
...
static int local_query_services(service_resolver * resolver,
                                service_resolver * peer,
                                struct service_desc *services, size_t num_svc);
static int local_query_services_async(service_resolver * resolver,
                                      service_resolver * peer,
                                      struct service_desc *services,
                                      size_t num_svc,
                                      service_resolver_callback * callback);

static int local_get_service_updates(service_resolver * resolver,
                                     service_resolver * peer,
                                     uint16_t type, struct service_desc *,
                                     size_t num_svc, stat_response * responses);
static int local_get_service_updates_async(service_resolver * resolver,
                                           service_resolver * peer,
                                           uint16_t type,
                                           struct service_desc *,
                                           size_t num_svc,
...
static int local_update_services(service_resolver * resolver,
                                 service_resolver * peer, uint16_t type,
                                 stat_response * responses);

static int local_resolve_service(service_resolver * resolver,
                                 service_resolver * peer,
                                 struct service_desc *service,
                                 struct net_addr *address);
static int local_resolve_service_async(service_resolver * resolver,
                                       service_resolver * peer,
                                       struct service_desc *service,
                                       struct net_addr *address,
                                       service_resolver_callback * callback);

static int local_poke_resolver(service_resolver * resolver,
                               service_resolver * peer, uint32_t count);
static int local_poke_resolver_async(service_resolver * resolver,
                                     service_resolver * peer,
                                     uint32_t count,
                                     service_resolver_callback * callback);
...
static service_resolver *local_get_peer(service_resolver * resolver,
                                        struct service_id *peer_id);

static int local_has_peer(service_resolver * resolver,
                          struct service_id *peer_id);
...
static int notify_peer_registration(struct sv_local_resolver *lres,
                                    service_resolver * peer,
                                    GArray * svc_list,
                                    struct net_addr *address, uint32_t ttl,
                                    int type);
static void purge_peer_resolution(struct sv_local_resolver *lres,
                                  service_resolver * peer,
                                        ...
                                        struct service_id *peer_id)
{
    ...
    struct peer_info *peer =
        (struct peer_info *) g_hash_table_lookup(lres->peer_table,
                                                 peer_id);
                          ...
                          struct service_id *peer_id)
{
    ...
    service_resolver *res =
        (service_resolver *) g_hash_table_lookup(lres->peer_table,
                                                 peer_id);
                                   ...
                                   uint32_t capabilities)
{
    ...
    hostctrl_set_capabilities(lres->lhc, capabilities);
                                  ...
                                  struct sockaddr_sv *sdesc)
{
    ...
    if (sdesc) {
        service_table_remove_service_reference(&lres->resolution_table,
                                               sdesc->sv_flags,
                                               sdesc->sv_prefix_bits,
                                               &sdesc->sv_srvid,
                                               resolver_get_address(peer,
                                                                    0), NULL);
    } else {
        struct service_id dummy;
        service_table_remove_service_reference(&lres->resolution_table, 0,
                                               0, &dummy,
                                               resolver_get_address(peer,
                                                                    0), NULL);
    }
...
static void local_echo_callback(service_resolver_callback * cb, int status,
                                void *data)
{
    assert(cb);
    ...
    if (status == -SV_TIMEOUT) {
        ...
        g_hash_table_remove(cbinfo->resolver->peer_table,
                            &cbinfo->pinfo->peer->resolver.
                            resolver_id.sv_srvid);
    ...
    if (pstatus > ACTIVE) {
        notify_peer_status_callbacks((service_resolver *) cbinfo->resolver,
                                     cbinfo->pinfo->peer, pstatus);
    }
...
static void local_echo_task(void *data)
{
    ...
    while (g_hash_table_iter_next(&iter, NULL, (void **) &rinfo)) {
        ...
        if (ctime - lastup > lres->echo_interval) {
            ...
            rinfo->peer->interface->poke_resolver_async(rinfo->peer,
                                                        &lres->
                                                        resolver.resolver, 0,
                                                        &cbinfo->cb);
        }
    ...
    if (g_hash_table_size(lres->peer_table) > 0) {
        ...
        task_add_delayed(&handle, local_echo_task, 
                         lres, lres->echo_interval * 1000);
    }
...
static void local_expire_task(void *data)
{
    ...
    struct service_reference *ref;
    struct service_info resolution;
    ...
    while (service_table_iter_next(&iter, &ref)) {
        ...
        if (ref->registered + ref->ttl * 1000 > ctime) {
            ...
            init_resolution_from_reference(&resolution, ref);
            ...
            if (service_table_iter_reference_count(&iter) == 1) {
                ...
                notify_peer_registration(lres, NULL, svc_list,
                                         (struct net_addr *) &ref->instance.
                                         address.sin.sin_addr, 0,
    ...
    if (ref_list->len > 0) {
        //
        hostctrl_services_remove(lres->lhc, (struct service_info *)
                                 ref_list->data,
                                 ref_list->len);
    }
    ...
    if (size > 0) {
        task_add_delayed(&lres->expire_task, local_expire_task, 
                         lres, lres->expire_interval * 1000);
    }
...
static void local_discover_task(void *data)
{
    ...
    /* default is to find peers on the local segment for now - do not
       propagate TODO */
    lres->default_peer->interface->peer_discovered(lres->default_peer,
                                                   &lres->resolver.resolver,
                                                   DISCOVER_NOTIFY);

    task_add_delayed(&lres->discover_task, local_discover_task, 
                     lres, lres->disc_interval * 1000);
} 
...
static int local_handle_service_unregistration(void *context,
                                               ...
                                               unsigned short prefix,
                                               const struct in_addr *ip);
static int local_handle_stat_update(void *context,
                                    const struct service_info_stat *res_stats, 
                                    size_t scount);
static int local_handle_resolve_service(void *context,
                                        struct service_desc *service,
                                        struct net_addr *address);

static struct hostctrl_callback hccb = {
    .service_registration = local_handle_service_registration,
    .service_unregistration = local_handle_service_unregistration,
};

service_resolver *create_local_service_resolver(struct sockaddr_sv *local,
                                                uint32_t capabilities,
                                                uint32_t capacity,
                                                service_resolver *default_res)
{
    struct sv_local_resolver *lres =
        (struct sv_local_resolver *) malloc(sizeof(*lres));
    ...
    memcpy(&lres->resolver.resolver.resolver.resolver_id, local,
           sizeof(struct sockaddr_sv));
    ...
    lres->lhc = hostctrl_local_create(&hccb, lres, 1);
                                               ...
                                               const struct in_addr *ip)
{
    ...
    sd.flags = flags;
    sd.prefix = prefix;
    return res->interface->unregister_services(res, NULL, &sd, 1, NULL);
}
                                    ...
                                    size_t scount)
{
    ...
    /*TODO this is rather dangerous to assume - full binary
      compatibilitye between service_info_stat == sv_instance_stats */
    resp.data = (uint8_t *) res_stats;
    return res->interface->update_services(res, NULL, SVS_INSTANCE_STATS,
                                           &resp);
}
...
static int local_initialize(service_resolver * resolver)
{
    ...
    lres->peer_table =
        g_hash_table_new_full(service_id_prefix_hash,
                              service_id_prefix_equal, NULL, destroy_peer_info);
...
static void local_stop(service_resolver * resolver)
{
    ...
    if (lres->discover_task) {
        task_cancel(lres->discover_task);
    }
...
static void local_register_callback(service_resolver_callback * cb,
                                   ...
                                   struct net_addr *address, uint32_t ttl)
{
    ...
    for (; i < num_svc; i++) {
        ...
        memcpy(&sref->instance.service.sv_srvid, &services[i].service,
        ...
        LOG_DBG("instance address: %s %s\n",
                inet_ntoa(sref->instance.address.sin.sin_addr),
        ...
        count =
            service_table_add_service_reference(&lres->resolution_table, sref);
        ...
        if (count >= 1) {
            ...
            if (ref_list) {
                ...
                memcpy(&sres.srvid, &sref->instance.service.sv_srvid,
                       sizeof(struct service_id));
                memcpy(&sres.address, &sref->instance.address.sin.sin_addr,
                       sizeof(struct net_addr));
    ...
    if (peer) {
         ...
         * this should follow some sort of policy - along with the
         * setting of default priority, weight, and timeouts */
        hostctrl_services_add(lres->lhc,(struct service_info *)
                              ref_list->data, ref_list->len);
        g_array_free(ref_list, TRUE);
    }

    if (rcount > 0 && (peer == NULL || peer != resolver)) {
        /* relies on an async callback, so the callback is reponsible
         * for freeing the svc_list */
        notify_peer_registration(lres, peer, svc_list, address, ttl,
                                 SV_REGISTER_REQUEST);
        g_array_unref(svc_list);
    } else {
        g_array_free(svc_list, TRUE);
                                     ...
                                     size_t num_svc, struct net_addr *address)
{
    ...
    if (peer) {
        hostctrl_services_remove(lres->lhc,
                                 (struct service_info *)
                                 ref_list->data,
                                 ref_list->len);
        g_array_free(ref_list, TRUE);
    ...
    if (svc_list) {
        if ((peer == NULL || peer != resolver)) {
            ...
            notify_peer_registration(lres, peer, svc_list, address, 0,
                                     SV_UNREGISTER_REQUEST);
            g_array_unref(svc_list);
        } else {
            g_array_free(svc_list, TRUE);
                                ...
                                struct service_desc *services, size_t num_svc)
{
    ...
    for (; i < num_svc; i++) {
        ...
        sdesc = &services[i];
        service_table_find_service_references(&lres->resolution_table,
                                              sdesc->flags, sdesc->prefix,
                                 ...
                                 stat_response * responses)
{
    ...
    case SVS_SERVICE_STATS:
        ...
        for (; i < responses->count; i++) {
            ...
            /* TODO -address?? */
            sref =
                service_table_find_service_reference(&lres->resolution_table,
                                                     stat->service.flags,
                                                     stat->service.prefix,
    ...
    case SVS_INSTANCE_STATS:
        ...
        for (; i < responses->count; i++) {
            ...
            sref =
                service_table_find_service_reference(&lres->resolution_table,
                                                     istat->service.flags,
                                                     istat->service.prefix,
                                                     &istat->service.service,
                                                     &istat->address);
                                     ...
                                     size_t num_svc, stat_response * responses)
{
    ...
    /*note that responses should already be allocated - count capped
      by message sizes */
    assert(resolver);
    ...
    int stat_index = 0;
    int resolutions = 0;
    /* really the only thing we're interested in receiving from a
       remote service is a service stat */
    task_rwlock_rdlock(&lres->res_lock);
    int i = 0;
    int j = 0;
    struct sv_instance_stats *istat = NULL;
    ...
    case SVS_INSTANCE_STATS:
        ...
        for (; i < num_svc; i++) {
            ...
            resolutions = hostctrl_service_query(lres->lhc,
                                                 &services[i].service,
                                                 services[i].flags,
                                                 services[i].prefix,
                                                 &sres);
        ...
        for (i = 0; i < num_svc && stat_index < responses->count; i++) {
            ...
            if (sref) {
                for (j = 0; j < count && stat_index < responses->count; j++) {
                    ...
                    /*take the max or min? */
                    sstat->duration_sec =
                        (get_current_time_ms() - sref[j]->registered) / 1000;
                    sstat->duration_nsec =
                        (get_current_time_ms() - sref[j]->registered)
                        * 1000000;

                    sstat->service.flags = sref[j]->instance.service.sv_flags;
                    sstat->service.prefix =
                        sref[j]->instance.service.sv_prefix_bits;
                    memcpy(&sstat->service.service,
                           &sref[j]->instance.service.sv_srvid,
                           sizeof(struct service_id));
    ...
    case SVS_ROUTER_STATS:
        ...
        service_table_get_table_stats(&lres->resolution_table,
                                      (struct sv_table_stats *) rstat);
        rstat->tables = 1;
                                 ...
                                 struct net_addr *address)
{
    ...
    LOG_DBG("Locally resolving service SID(%i:%i) %s\n", service->flags,
            service->prefix, service_id_to_str(&service->service));
    ...
    sref = service_table_resolve_service(&lres->resolution_table,
                                         service->flags, service->prefix,
                                         &service->service);
...
static int notify_discovery(struct sv_local_resolver *lres,
                            struct peer_info *pinfo, uint16_t flags)
{
    /* assumes that last_discovery has been updated already */

    if (pinfo->last_discovery > pinfo->disc_cooldown) {
        pinfo->disc_cooldown =
            pinfo->last_discovery + DISCOVERY_COOLDOWN * 1000;
        pinfo->peer->interface->peer_discovered(pinfo->peer,
                                                &lres->resolver.resolver,
                                                flags);
    }
...
static int propagate_discovery(struct sv_local_resolver *lres,
                               struct peer_info *pinfo, uint16_t flags)
{
    /* assumes that last_discovery has been updated already
    ...
    uint16_t prel = 0;
    if (rel == RELATION_CHILD || rel == RELATION_SELF
        || rel == RELATION_UNKNOWN || rel == RELATION_PARENT) {
        return 0;
    }
...
static int should_retain_peer(struct sv_local_resolver *lres,
                              service_resolver * peer)
{
...
static void add_peer_resolution(struct sv_local_resolver *lres,
                                service_resolver * peer,
                                struct sockaddr_sv *sdesc)
{
    struct service_reference *ref =
        (struct service_reference *) malloc(sizeof(*ref));
    bzero(ref, sizeof(*ref));
                                 ...
                                 service_resolver *peer, uint16_t flags)
{
    ...
    if (g_hash_table_lookup_extended
        (lres->peer_table, &peer->resolver.resolver_id.sv_srvid, NULL,
         (void **) &pinfo)) {
        ...
        /*decide whether or not to add in the peer based on
        ...
        if (g_hash_table_size(lres->peer_table) == 0) {
            task_add_delayed(&lres->echo_task, local_echo_task, 
                             lres, lres->echo_interval * 1000);
        }

        g_hash_table_insert(lres->peer_table,
                            &peer->resolver.resolver_id.sv_srvid, pinfo);
                                  ...
                                  service_resolver * peer, uint16_t flags)
{
    ...
    if (g_hash_table_lookup_extended
        (lres->peer_table, &peer->resolver.resolver_id.sv_srvid, NULL, NULL)) {
        /* the peer has been updated - its still good to respond in case he's forgotten about us
         * might need to "limit" the # of response and propagations per peer - NACK table?
         * */
