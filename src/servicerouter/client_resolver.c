...
struct sv_client_resolver {
    ...
    uint32_t uptime;
    task_mutex_t message_mutex;
    task_cond_t message_cond;
                               ...
                               struct sockaddr *saddr, size_t len)
{
    ...
    struct sv_client_resolver *clientres =
        (struct sv_client_resolver *) resolver;
    message_channel_set_peer(clientres->messaging->resolver.channel, 
                             saddr, len);
                                     ...
                                     service_resolver_callback * cb)
{
    ...
    /* packetize services into requests */
    uint16_t max_services = (resolver_rpc_get_max_message_size((resolver_rpc *) clientres->messaging) - sizeof(struct sv_register_message)) / sizeof(struct service_desc);
