...
extern struct message_channel *
create_udp_message_channel(struct sockaddr_sv *local, 
                           struct sv_instance_addr *remote, 
                           int buffer_len, 
                           struct message_channel_callback *callback);
...
static void set_local_address(resolver_rpc * messenger,
                              struct sockaddr *saddr, size_t len);

static void set_callback(resolver_rpc * messenger,
                         resolver_message_callback * callback);
...
static int send_resolver_request(resolver_rpc *resm, uint32_t xid,
                                 ...
                                 size_t len, resolver_message_callback *cb);
...
struct rpc_expire_set {
    ...
    task_mutex_t mutex;
    void (*expire_requests) (resolver_rpc * bm, uint64_t curtime,
                             struct rpc_expire_set * eset);
};

static int handle_request_packet(resolver_rpc * bm,
                                 struct sv_control_header *message, 
                                 size_t len);

static int handle_response_packet(resolver_rpc * bm,
                                  struct sv_control_header *message,
                                  size_t len);

static int resolver_client_callback(struct message_channel_callback *cb,
                                    message_t *msg);
static int resolver_server_callback(struct message_channel_callback *cb,
                                    message_t *msg);
...
static void expire_client_requests(resolver_rpc * bm, uint64_t curtime,
                                   struct rpc_expire_set *eset);
static void expire_server_requests(resolver_rpc * bm, uint64_t curtime,
                                   struct rpc_expire_set *eset);
                                  ...
                                  uint64_t expiration)
{
    ...
    if (expiration < eset->expiration && eset->rpc_list->len > 0) {
        ...
        /*also reschedules */
        task_add_delayed(&eset->task_handle, expire_requests, eset, expiration);
    }
...
static void register_resolver_rpc(resolver_rpc * bm,
                                  struct rpc_expire_set *eset)
{
    assert(bm && eset);
...
static void unregister_resolver_rpc(resolver_rpc * bm,
                                    struct rpc_expire_set *eset)
{
    assert(bm && eset);
...
resolver_rpc *create_resolver_rpc(struct sockaddr_sv *local_resolver,
                                  struct sv_instance_addr *remote_resolver,
                                  uint16_t max_retry,
                                  uint16_t request_timeout,
                                  resolver_message_callback * cb)
{
    ...
    /*
    struct message_channel_callback mcb;
    mcb.target = bm;

    if (cb) {
        mcb.recv = resolver_server_callback;
    } else {
        mcb.recv = resolver_client_callback;
    }
    */
    /*note that the channel can/should be shared between server/client
      for a given remote peer */
    /*
    if (!(bm->resolver.channel =
          create_udp_message_channel(local_resolver, remote_resolver, 0,
                                     &mcb))) {
...
static void set_local_address(resolver_rpc *bm, struct sockaddr *saddr,
                              size_t len)
{
    assert(bm);
    ...
    //needs to stop and recreate the message /* channel p */- if the addr has really changed
    if (bm->resolver.channel) {
        struct message_channel *channel = bm->resolver.channel;
        struct message_channel_callback cb;
        socklen_t llen, plen;
        struct sv_instance_addr laddr, paddr;
        
        message_channel_get_local(channel, (struct sockaddr *)&laddr, &llen);
        ...
        if (bm->resolver.callback.target) {
            cb.recv = resolver_server_callback;
        } else {
            cb.recv = resolver_client_callback;
        }

        message_channel_unregister_callback(channel, &cb);
        
        message_channel_get_peer(bm->resolver.channel,
                                 (struct sockaddr *)&paddr, &plen);        

        /*note that the channel can/should be shared between
          server/client for a given remote peer */
        if (!(bm->resolver.channel = 
              message_channel_get_generic(MSG_CHANNEL_UDP,
                                          SOCK_DGRAM,
                                          0,
                                          saddr,
                                          len,
        ...
        message_channel_register_callback(bm->resolver.channel, &cb);
        message_channel_put(channel);
    }
...
static int finalize(resolver_rpc * bm)
{
    ...
    if (bm->resolver.channel) {
         ...
         */
        message_channel_unregister_callback(bm->resolver.channel, &cb);

        message_channel_put(bm->resolver.channel);
...
static int resolver_message_initialize(resolver_message * message)
{
    ...
    if (message->message_len > 0) {
        ...
        message->message =
            (struct sv_control_header *) malloc(message->message_len);
        /* TODO - mem check? */
    }
...
static void expire_requests(void *data)
{
    ...
    if (eset->expiration > 0) {
        //reschedule
        task_handle_t th;
        task_add_delayed(&th, expire_requests, eset, eset->expiration);
    }
                                   ...
                                   struct rpc_expire_set *eset)
{
 ...
 out:
    if (bm->resolver.outstanding_requests > 0 && eset->expiration == 0) {
                                   ...
                                   struct rpc_expire_set *eset)
{
        ...
        if (message->exp_time <= curtime) {
            if (message->retries >= bm->resolver.max_retry) {
                ...
                if (message->message != NULL) {
                    ...
                    message->callback.resolver_message_cb(message->callback.
                                                          target, SV_TIMEOUT,
                                                          message->message,
                                                          message->message_len,
                                                          &peer_addr);
 ...
 out:
    ...
    if (bm->resolver.outstanding_requests > 0
        && (eset->expiration == 0 || eset->expiration > mintime)) {
        /* set it */
    ...
    for (; i < resend->len; i++) {
        ...
        res = message_channel_send(bm->resolver.channel, 
    ...
    LOG_DBG("Re-issued %i rpc requests at: %" PRIu64 " mintime: %" PRIu64
            " ", count, curtime, mintime);
...
static int resolver_client_callback(struct message_channel_callback *cb,
                                    message_t *msg)
{
    return handle_response_packet((resolver_rpc *) cb->target,
                                  (struct sv_control_header *) msg->data, 
                                  msg->length);
}
...
static int resolver_server_callback(struct message_channel_callback *cb,
                                    message_t *msg)
{
    return handle_request_packet((resolver_rpc *) cb->target,
                                 (struct sv_control_header *)msg->data, 
                                 msg->length);
}

static int check_resolver_message(resolver_rpc *bm,
                                  struct sv_control_header *message,
                                  size_t len, int req_mode)
{
                                 ...
                                 struct sv_control_header *message, size_t len)
{
    ...
    resolver_message *rmessage =
        (resolver_message *) g_hash_table_lookup(bm->resolver.rpc_request_map,
                                                 &xid);
    ...
    if (rmessage) {
        ...
        if (rmessage->message) {
            retval = message_channel_send(bm->resolver.channel,
                                          rmessage->message,
                                          rmessage->message_len);
        } else {
        ...
        if (type == SV_UPDATE_REQUEST || type == SV_REGISTER_REQUEST
            || type == SV_UNREGISTER_REQUEST || type == SV_QUERY_REQUEST) {
            /* new request */
            LOG_DBG("Handling serval protocol request: %u", type);
            rmessage = (resolver_message *) malloc(sizeof(resolver_message));

            //assert?
            if (rmessage == NULL) {
                LOG_ERR
                    ("Could not allocate a resolver_message - dropping request");
                retval = -1;
                goto unlockout;
            }
            ...
            rmessage->exp_time =
                bm->resolver.last_remote_access +
                bm->resolver.request_timeout * 1000;
        ...
        /* another option is to push this onto the task queue for
         * async handling which could be determined adaptively based
         * on resptime - would require copying the message body
         */
        message_channel_get_peer(bm->resolver.channel, 
                                 (struct sockaddr *)&peer_addr, 
                                 &addrlen);
        
        bm->resolver.callback.resolver_message_cb(&bm->resolver.callback,
                                                  type, message, len,
                                                  &peer_addr);
    }
    goto out;
                                  ...
                                  struct sv_control_header *message, size_t len)
{
    ...
    if (rmessage) {
        if (!g_hash_table_remove(bm->resolver.rpc_request_map, 
                                 &rmessage->xid)) {
            LOG_ERR("Remove error on response for rpc request: %u",
                    rmessage->xid);
        }
        ...
        if (rmessage->callback.target) {
            ...
            /* another option is to push this onto the task queue for
             * async handling which could be determined adaptively
             * based on resptime - would require copying the message
             * body
             */
            message_channel_get_peer(bm->resolver.channel,
                                     (struct sockaddr *)&peer_addr,
            ...
            rmessage->callback.resolver_message_cb(rmessage->callback.target,
                                                   message->type, message, len,
                                                   &peer_addr);
        } else {
            LOG_ERR
                ("No default response callback defined - dropping rpc response: %u",
                 xid);
            retval = -1;
        }
...
static int send_resolver_response(resolver_rpc * bm, uint32_t xid,
                                  struct sv_control_header *response,
                                  size_t len, resolver_message_callback * rm_cb)
{
    ...
    rmessage =
        g_hash_table_lookup(bm->resolver.rpc_request_map, (const void *) &xid);

    if (rmessage == NULL) {
        if (type != SV_UPDATE_MESSAGE) {
            //error - response to non-existent request
            LOG_ERR("No request entry found for %u xid, message type: %u",
                    xid, type);
            retval = -1;
        }
        goto unlock;
    ...
    if (rmessage->message) {
        /* response already exists - another error */
        LOG_ERR
            ("A response has already been sent for %u xid, type: %u", xid,
             type);
        retval = -1;
        goto unlock;
    }
 ...
 unlock:
    ...
    return message_channel_send(bm->resolver.channel, response, len);
}

static int send_resolver_request(resolver_rpc * bm, uint32_t xid,
                                 struct sv_control_header *request,
                                 size_t len, resolver_message_callback * rm_cb)
{
    assert(bm);
    ...
    if (!(type == SV_DISCOVER_MESSAGE || type == SV_UPDATE_MESSAGE)) {
        ...
        if ((rmessage =
             g_hash_table_lookup(bm->resolver.rpc_request_map,
                                 (const void *) &xid))) {
            //error - response to non-existent request
    ...
    if (retval == 0) {
        retval = message_channel_send(bm->resolver.channel, request, len);
    }
    //request with id to host exists - rely on retry!
    return retval;
}

static void set_callback(resolver_rpc *bm,
                         resolver_message_callback *callback)
{
    assert(bm);
    task_mutex_lock(&bm->resolver.req_mutex);
    ...
    if (bm->resolver.callback.target == NULL) {
        if (callback) {
            ...
            if (message_channel_register_callback(bm->resolver.channel, &mcb)) {
        ...
        message_channel_callback_t mcb;

        bm->resolver.callback.target = NULL;
        mcb.target = bm;
        mcb.recv = resolver_server_callback;

        message_channel_unregister_callback(bm->resolver.channel, &mcb);
        unregister_resolver_rpc(bm, &server_expiry);
    } else {
        bm->resolver.callback = *callback;
    }
...
void resolver_rpc_set_peer(resolver_rpc * bm,
                           struct sv_instance_addr *peer_addr)
{
    assert(bm);
    if (peer_addr == NULL) {
    ...
    /* only place to lock both locks? TODO - going to be hard to debug
       with coroutines... */
    task_mutex_lock(&bm->resolver.req_mutex);
    message_channel_set_peer(bm->resolver.channel,
                             (struct sockaddr *)peer_addr,
                             sizeof(*peer_addr));
    task_mutex_lock(&bm->resolver.req_mutex);
}

int resolver_rpc_get_peer(resolver_rpc *bm, struct sockaddr *addr, 
                          socklen_t *addrlen)
{
    int ret;
    ...
    ret = message_channel_get_peer(bm->resolver.channel, addr, addrlen);
...
int resolver_rpc_get_max_message_size(resolver_rpc * bm)
{
    ...
    task_mutex_lock(&bm->resolver.req_mutex);
    max = message_channel_get_max_message_size(bm->resolver.channel);
    task_mutex_lock(&bm->resolver.req_mutex);
    return max;
}
