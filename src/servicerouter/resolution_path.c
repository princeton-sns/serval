...
static void handle_resolve_message(resolution_path * spath,
				   ...
				   size_t length);
static void handle_resolution_removed(resolution_path * spath, struct ctrlmsg_service_stat
						...
						size_t len)
{
    ...
    struct ctrlmsg_service_stat *smessage =
	(struct ctrlmsg_service_stat *) msg;
			     ...
			     struct service_stat *stats)
{
    ...
    cm.cmh.type = CTRLMSG_TYPE_SERVICE_STAT;
					      ...
					      size_t length)
{
	...
	handle_resolution_removed(spath,
				  (struct ctrlmsg_service_stat *) message,
				  length);
	break;
