...
static void server_resolver_message_cb(resolver_message_callback * cb,
                                       uint16_t type,
                                       struct sv_control_header *message,
                                       size_t len,
                                       struct sv_instance_addr *remote);
static void server_resolver_peer_status_cb(struct sv_peer_status_callback
                                           *cb, service_resolver * peer,
...
struct server_rpc_handler *create_server_rpc_handler(service_resolver * res)
{
    ...
    struct server_rpc_handler *handler =
        (struct server_rpc_handler *) malloc(sizeof(*handler));
                            ...
                            service_resolver * res)
{
    ...
    memcpy(&defaddr.service, &service_router_prefix,
           sizeof(struct sockaddr_sv));

    if (!(handler->def_callback.rpc =
          create_resolver_rpc(&res->resolver.resolver_id, &defaddr, 0, 1,
                              &handler->callback))) {
...
int server_rpc_handler_finalize(struct server_rpc_handler *handler)
{
    ...
    g_ptr_array_remove_range(handler->callback_list, 0,
                             handler->callback_list->len);
    g_ptr_array_free(handler->callback_list, TRUE);

    callback_info_finalize(&handler->def_callback);

    resolver_unregister_peer_status_callback(handler->resolver,
                                             &handler->status_callback);
    return 0;
}
...
static void handle_discover_message(struct server_rpc_handler *handler,
                                    ...
                                    size_t len, struct sv_instance_addr *remote)
{
    ...
    struct sv_discover_message *dmessage =
        (struct sv_discover_message *) message;
    uint32_t cap = ntohl(dmessage->capabilities);
    uint8_t rel = RELATION_UNKNOWN;
    int created = FALSE;

    /*easy relation check - local is transit, remote is stub */
    if (is_transit(handler->resolver->resolver.capabilities)
        && is_stub(cap)) {
        rel = RELATION_CHILD;
    } else if (is_stub(handler->resolver->resolver.capabilities)
               && is_transit(cap)) {
        rel = RELATION_PARENT;
    } else if (is_stub(handler->resolver->resolver.capabilities)
               && is_stub(cap)) {
        ...
        LOG_DBG("Stub resolver %s dropping peer discovery %s\n",
                service_id_to_str(&handler->resolver->resolver.
    ...
    if (peer == NULL) {
        ...
        if (dmessage->resolver_id.flags != SVSF_INVALID) {
            ...
            memcpy(&peeraddr.service.sv_srvid,
                   &dmessage->resolver_id.service, sizeof(struct service_id));

            peeraddr.address.sin.sin_family = AF_INET;
            memcpy(&peeraddr.address.sin.sin_addr,
                   &dmessage->resolver_addr.net_ip, sizeof(struct in_addr));
        ...
        if (!
            (cinfo->peer =
             create_client_service_resolver(&((struct sv_service_resolver *)
                                              handler->resolver)->resolver_id,
                                            remote, ntohl(dmessage->uptime),
                                            cap, ntohl(dmessage->capacity),
                                            rel))) {
        ...
        resolver_add_address(cinfo->peer,
                             (struct net_addr *) &remote->address.sin.sin_addr);
            ...
            (cinfo->rpc =
             create_resolver_rpc(&
                                 ((struct sv_service_resolver *)
                                  handler->resolver)->resolver_id, remote,
    ...
    int retval =
        handler->resolver->interface->peer_discovered(handler->resolver,
                                                      peer,
...
static void send_error_reply(resolver_rpc * rpc, uint32_t xid,
                             uint16_t err_type, uint16_t msg_type,
                             const char *message)
{

    int msize = message == NULL ? 0 : strlen(message);
    struct sv_error_reply *emsg =
        (struct sv_error_reply *) malloc(sizeof(*emsg) + msize);
    ...
    rpc->interface->send_resolver_message(rpc, xid, &emsg->header,
                                          sizeof(*emsg), NULL);
}

static void handle_disappear_message(struct server_rpc_handler *handler,
                                     service_resolver * peer,
                                     resolver_rpc * rpc,
                                     struct sv_control_header *message,
                                     size_t len,
                                     struct sv_instance_addr *remote)
{
    ...
    struct sv_disappear_message *dmessage =
        (struct sv_disappear_message *) message;

    if (peer == NULL) {
        /* send error */
        send_error_reply(rpc, ntohl(message->xid), SV_ERR_UNAUTHORIZED,
                         ntohs(message->type), "Peer resolver unknown");
        return;
    }
    ...
    if (resolver_get_address_count(peer) == 0) {
        ...
        handler->resolver->interface->peer_disappeared(handler->resolver,
                                                       peer,
                                                       ntohs(dmessage->flags));
    }
...
static void handle_echo_request(struct server_rpc_handler *handler,
                                service_resolver * peer,
                                resolver_rpc * rpc,
                                struct sv_control_header *message,
                                size_t len, struct sv_instance_addr *remote)
{
    ...
    int retval = handler->resolver->interface->poke_resolver(handler->resolver,
                                                             peer, count);
    ...
    if (retval < 0) {
        ...
        init_control_header(&emessage->header, SV_ECHO_REPLY, xid,
                            sizeof(*emessage));
        emessage->count = htonl(count);

        retval =
            rpc->interface->send_resolver_message(rpc, xid,
                                                  &emessage->header,
                                                  sizeof(*emessage), NULL);
    }
}

static void handle_register_request(struct server_rpc_handler *handler,
                                    service_resolver * peer,
                                    resolver_rpc * rpc,
                                    struct sv_control_header *message,
                                    size_t len, struct sv_instance_addr *remote)
{
    ...
    peer->interface->incref(peer);
    int retval = handler->resolver->
        interface->register_services(handler->resolver,
                                     peer,
                                     rmessage->service_ids,
                                     scount,
                                     &rmessage->address,
                                     ntohl(rmessage->ttl));
    ...
    if (retval < 0) {
        ...
        retval =
            rpc->interface->send_resolver_message(rpc, xid, message,
                                                  sizeof(*message), NULL);
    }

}

static void handle_unregister_request(struct server_rpc_handler *handler,
                                      service_resolver * peer,
                                      resolver_rpc * rpc,
                                      struct sv_control_header *message,
                                      size_t len,
                                      struct sv_instance_addr *remote)
{
    ...
    peer->interface->incref(peer);
    int retval =
        handler->resolver->interface->unregister_services(handler->resolver,
                                                          peer,
                                                          rmessage->service_ids,
                                                          scount,
                                                          &rmessage->address);
...
static void handle_update_request(struct server_rpc_handler *handler,
                                  service_resolver * peer,
                                  resolver_rpc * rpc,
                                  struct sv_control_header *message,
                                  size_t len, struct sv_instance_addr *remote)
{
    ...
    struct sv_update_message *smessage =
        (struct sv_update_message *) malloc(size);
    ...
    peer->interface->incref(peer);
    int retval =
        handler->resolver->interface->get_service_updates(handler->resolver,
                                                          peer,
                                                          stype,
                                                          umessage->service_ids,
                                                          scount, &resp);
    ...
    if (retval <= 0) {
        ...
        for (i = 0; i < retval; i++) {
            prep_stats_for_network(stype, smessage->body + (i * statsize),
                                   resp.data + (i * statsize));
        }
        ...
        retval =
            rpc->interface->send_resolver_message(rpc, xid,
                                                  &smessage->header, size,
                                                  NULL);
    }

}

static void handle_update_message(struct server_rpc_handler *handler,
                                  service_resolver * peer,
                                  resolver_rpc * rpc,
                                  struct sv_control_header *message,
                                  size_t len, struct sv_instance_addr *remote)
{
    ...
    int i = 0;
    for (; i < scount; i++) {
        prep_stats_for_host(stype, resp.data + (i * statsize),
                            umessage->body + (i * statsize));
    }

    peer->interface->incref(peer);
    int retval =
        handler->resolver->interface->update_services(handler->resolver,
                                                      peer, stype,
                                                      &resp);
...
static void handle_query_request(struct server_rpc_handler *handler,
                                 service_resolver * peer,
                                 resolver_rpc * rpc,
                                 struct sv_control_header *message,
                                 size_t len, struct sv_instance_addr *remote)
{
    ...
    peer->interface->incref(peer);
    int retval = handler->resolver->interface->query_services(handler->resolver,
                                                              peer,
                                                              qmessage->
                                                              service_ids,
                                                              scount);
    ...
    if (retval <= 0) {
        ...
        /* is this correct here is it always aysnc? TODO */
        int size =
            sizeof(struct sv_query_response) +
            scount * sizeof(struct service_desc);
        struct sv_query_response *qresp =
            (struct sv_query_response *) malloc(size);
        bzero(qresp, sizeof(*qresp));
        init_control_header(&qresp->header, SV_QUERY_REPLY, xid, size);
        memcpy(&qresp->service_ids, &qmessage->service_ids,
               scount * sizeof(struct service_desc));
        ...
        retval =
            rpc->interface->send_resolver_message(rpc, xid, &qresp->header,
                                                  size, NULL);
    }
}

static void handle_resolution_request(struct server_rpc_handler *handler,
                                      service_resolver * peer,
                                      resolver_rpc * rpc,
                                      struct sv_control_header *message,
                                      size_t len,
                                      struct sv_instance_addr *remote)
{
    ...
    struct sv_resolution_request *rmessage =
        (struct sv_resolution_request *) message;
    ...
    peer->interface->incref(peer);
    int retval =
        handler->resolver->interface->resolve_service(handler->resolver,
                                                      peer,
                                                      &rmessage->service_id,
                                                      &addr);
    ...
    if (retval <= 0) {
        ...
        struct sv_resolution_response *rresp =
            (struct sv_resolution_response *) malloc(sizeof(*rresp));
        bzero(rresp, sizeof(*rresp));
        init_control_header(&rresp->header, SV_RESOLUTION_REPLY, xid,
                            sizeof(*rresp));

        memcpy(&rresp->service_id, &rmessage->service_id,
               sizeof(struct service_desc));
        memcpy(&rresp->address, &addr, sizeof(addr));
        rresp->service_id.type = htons(rmessage->service_id.type);

        retval =
            rpc->interface->send_resolver_message(rpc, xid, &rresp->header,
                                                  sizeof(*rresp), NULL);
    }
}

static int find_cinfo_by_peer(struct server_rpc_handler *handler,
                              service_resolver * peer)
{
    assert(handler);
    ...
    for (i = 0; i < handler->callback_list->len; i++) {
        cinfo =
            (struct callback_info *) g_ptr_array_index(handler->callback_list,
                                                       i);
        if (cinfo->peer == peer) {
            return i;
        }
                                           ...
                                           enum resolver_state state)
{
    ...
    struct server_rpc_handler *handler =
        (struct server_rpc_handler *) cb->target;
    ...
    if (state >= UNRESPONSIVE) {
        ...
        if (index < 0) {
            LOG_ERR
                ("Could not find peer %s that should have been in the cinfo list!\n",
                                       ...
                                       struct sv_instance_addr *remote)
{
    ...
    case SV_DISCOVER_MESSAGE:
        handle_discover_message(cinfo->handler, cinfo->peer, cinfo->rpc,
                                message, len, remote);
        return;
    case SV_DISAPPEAR_MESSAGE:
        handle_disappear_message(cinfo->handler, cinfo->peer, cinfo->rpc,
                                 message, len, remote);
        return;
    case SV_ECHO_REQUEST:
        handle_echo_request(cinfo->handler, cinfo->peer, cinfo->rpc,
                            message, len, remote);
        return;
        //service registration - authorization and authentication
    case SV_REGISTER_REQUEST:
        handle_register_request(cinfo->handler, cinfo->peer, cinfo->rpc,
                                message, len, remote);
        return;
    case SV_UNREGISTER_REQUEST:
        handle_unregister_request(cinfo->handler, cinfo->peer, cinfo->rpc,
                                  message, len, remote);
        return;
        //service update (load and meta/stat info)
    case SV_UPDATE_REQUEST:
        handle_update_request(cinfo->handler, cinfo->peer, cinfo->rpc,
                              message, len, remote);
        return;
    case SV_UPDATE_MESSAGE:
        handle_update_message(cinfo->handler, cinfo->peer, cinfo->rpc,
                              message, len, remote);
        return;
        //service query
    case SV_QUERY_REQUEST:
        handle_query_request(cinfo->handler, cinfo->peer, cinfo->rpc,
                             message, len, remote);
        return;
    case SV_RESOLUTION_REQUEST:
        handle_resolution_request(cinfo->handler, cinfo->peer, cinfo->rpc,
                                  message, len, remote);
        return;
    default:
