...
typedef struct sv_resolver_callback {
    void *target;
    void (*service_resolver_cb) (struct sv_resolver_callback * cb,
...
typedef struct sv_peer_status_callback {
    void *target;
    void (*peer_status_cb) (struct sv_peer_status_callback * cb,
...
struct sv_resolver_interface {
    ...
    void (*set_address) (service_resolver * resolver,
                         struct sockaddr * saddr, size_t len);
    void (*set_capabilities) (service_resolver * resolver,
                              uint32_t capabilities);
    /* get/set state? - locked? */
    service_resolver *(*get_peer) (service_resolver * resolver,
    ...
    int (*peer_discovered) (service_resolver * resolver,
                            service_resolver * peer, uint16_t type);

    int (*peer_disappeared) (service_resolver * resolver,
                             service_resolver * peer, uint16_t type);
    ...
    int (*register_services) (service_resolver *resolver,
    ...
    int (*register_services_async) (service_resolver *resolver,
                                    service_resolver *peer,
                                    struct service_desc *services,
                                    size_t num_svc,
                                    struct net_addr *address,
    ...
    int (*unregister_services) (service_resolver *resolver,
                                service_resolver *peer,
                                struct service_desc *services,
                                size_t num_svc, struct net_addr *address);
    int (*unregister_services_async) (service_resolver *resolver,
                                      service_resolver *peer,
                                      struct service_desc *services,
                                      size_t num_svc,
                                      struct net_addr *address,
    ...
    int (*query_services) (service_resolver *resolver,
                           service_resolver *peer,
                           struct service_desc *services, size_t num_svc);
    int (*query_services_async) (service_resolver *resolver,
                                 service_resolver *peer,
                                 struct service_desc *services,
                                 size_t num_svc,
                                 service_resolver_callback * callback);

    int (*update_services) (service_resolver * resolver,
                            service_resolver * peer, uint16_t type,
                            stat_response * responses);

    int (*get_service_updates) (service_resolver * resolver,
                                service_resolver * peer, uint16_t type,
                                struct service_desc *, size_t num_svc,
                                stat_response * responses);

    int (*get_service_updates_async) (service_resolver * resolver,
                                      service_resolver * peer,
    ...
    int (*resolve_service) (service_resolver * resolver,
                            service_resolver * peer,
                            struct service_desc * service,
                            struct net_addr * address);

    int (*resolve_service_async) (service_resolver * resolver,
                                  service_resolver * peer,
                                  struct service_desc * service,
                                  struct net_addr * address,
                                  service_resolver_callback * callback);

    int (*poke_resolver) (service_resolver * resolver,
                          service_resolver * peer, uint32_t count);
    int (*poke_resolver_async) (service_resolver * resolver,
                                service_resolver * peer, uint32_t count,
                                service_resolver_callback * callback);
