...
public class TestApp {
	...
	try {
	    hc = new LocalHostCtrl(new HostCtrlCallbacks() {
		    ...
		    @Override
			public void onServiceRegistration(ServiceID id, 
							  int flags,
							  InetAddress addr, 
							  InetAddress oldAddr) {
		    ...
		    @Override
			public void onServiceUnregistration(ServiceID id, 
							    int flags,
							    InetAddress addr) {
	...
	try {
	    ...
	    hc.addService(new ServiceID("1.testapp.localdomain"), addr);
			
	    Thread.sleep(1000);
			
	    hc.getService(new ServiceID("2.testapp.localdomain"), null);
			
	    System.out.println("Waiting 3 seconds for events...");
			
	    Thread.sleep(3000);
			
	    hc.removeService(new ServiceID("3.testapp.localdomain"), addr);
