...
public abstract class HostCtrl {
	...
	private native int getService4(ServiceID id,
			Inet4Address addr);

	private native int removeService4(ServiceID id,
			Inet4Address addr);

	private native int registerService4(ServiceID id,
	...
	public int addService(ServiceID id, int priority,
		...
		return addService4(SERVICE_RULE_FORWARD, id, priority,
	...
	public int addServiceDelay(ServiceID id, int priority, int weight) {
		return addService4(SERVICE_RULE_DELAY, id, priority,
				weight, null);
	}
	
	public int addServiceDrop(ServiceID id, int priority, int weight) {
 		return addService4(SERVICE_RULE_DROP, id, priority,
				weight, null);
	}
	
	public int addService(final int type, ServiceID id,
			int priority, int weight, InetAddress addr) {
		return addService4(type, id, priority, weight, (Inet4Address) addr);
	}
