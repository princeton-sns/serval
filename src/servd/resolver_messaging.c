...
extern message_channel *create_udp_message_channel(struct sockaddr_sv
						   *local, struct sv_instance_addr
...
static int resolver_client_callback(message_channel_callback * cb,
				    const void *message, size_t len);
static int resolver_server_callback(message_channel_callback * cb,
				    const void *message, size_t len);
			      ...
			      size_t len)
{
	...
	const struct sockaddr *laddr =
	    bm->resolver.channel->interface->
	    get_local_address(bm->resolver.channel,
			      &llen);
	...
	bm->resolver.channel->interface->unregister_callback(bm->
							     resolver.channel,
							     &cb);

	if (bm->resolver.channel->
	    interface->get_callback_count(bm->resolver.channel) == 0) {
	...
	if (!
	    (bm->resolver.channel =
	     create_udp_message_channel((struct sockaddr_sv *) saddr,
					(struct sv_instance_addr *)
					bm->resolver.channel->
					interface->get_peer_address(bm->
								    resolver.channel,
								    &llen), 0,
					&cb))) {
	    LOG_ERR("Could not create resolver rpc - message channel error\n");
				   ...
				   struct rpc_expire_set *eset)
{
		...
		if (message->message != NULL) {
		    ...
		    message->callback.resolver_message_cb(message->callback.
							  ...
							  (struct
							   sv_instance_addr *)
							  bm->
							  resolver.channel->interface->get_peer_address
							  (bm->resolver.channel,
							   &len));
		}
  ...
  out:
	...
	res =
	    bm->resolver.channel->interface->send_message(bm->resolver.channel,
							  message->message,
							  message->message_len);
	if (res) {
				 ...
				 struct sv_control_header *message, size_t len)
{
	...
	if (rmessage->message) {
	    retval =
		bm->resolver.channel->interface->send_message(bm->
							      resolver.channel,
							      rmessage->message,
				  ...
				  size_t len, resolver_message_callback * rm_cb)
{
  ...
  unlock:task_mutex_unlock(&bm->resolver.req_mutex);
    ...
    return bm->resolver.channel->interface->send_message(bm->resolver.channel,
							 response, len);
}
			 ...
			 resolver_message_callback * callback)
{
	...
	if (callback) {
	    ...
	    if (bm->resolver.channel->
		interface->register_callback(bm->resolver.channel, &mcb)) {
		LOG_DBG
		    ("Error registering channel callback for resolver messaging\n");
		return;
	    }
...
const struct sv_instance_addr *resolver_rpc_get_peer(resolver_rpc * bm)
{
    ...
    int len;
    peer =
	(struct sv_instance_addr *) bm->resolver.channel->
	interface->get_peer_address(bm->resolver.channel, &len);
    task_mutex_lock(&bm->resolver.req_mutex);
    return peer;
}
...
int resolver_rpc_get_max_message_size(resolver_rpc * bm)
{
    ...
    int max;
    max =
	bm->resolver.channel->interface->get_max_message_size(bm->
							      resolver.channel);
    task_mutex_lock(&bm->resolver.req_mutex);
    return max;
}
