				  ...
				  struct sockaddr_sv *sdesc)
{
    ...
    memcpy(&res.service.address, resolver_get_address(peer, 0),
	   sizeof(struct net_addr));
...
static int local_handle_path_service_registered(resolution_path_callback * cb, struct service_desc
						*service);
static int local_handle_path_service_unregistered(resolution_path_callback
						  * target, struct service_desc
						  *service);
static int local_handle_path_stat_update(resolution_path_callback * target, struct service_info_stat
					 *res_stats, size_t scount);
static int local_handle_path_resolve_service(resolution_path_callback *
					     target,
					     struct service_desc *service,
					     struct net_addr *address);
						...
						resolution_path * spath)
{
    ...
    lres->path_callback.service_registered =
	local_handle_path_service_registered;
    lres->path_callback.service_unregistered =
	local_handle_path_service_unregistered;
...
static int local_handle_path_service_unregistered(resolution_path_callback * cb, struct service_desc
						  *service)
{
    assert(cb);
    service_resolver *res = (service_resolver *) cb->target;
...
static int local_handle_path_stat_update(resolution_path_callback * cb, struct service_info_stat
					 *res_stats, size_t scount)
{
    assert(cb);
    service_resolver *res = (service_resolver *) cb->target;
				     ...
				     size_t num_svc, stat_response * responses)
{
	...
	for (; i < num_svc; i++) {
	    ...
	    resolutions =
		lres->rpath->interface->get_resolutions(lres->rpath,
							&services[i],
							(struct service_info **)
							&sres);
				 ...
				 struct net_addr *address)
{
    ...
    sref =
	service_table_resolve_service(&lres->resolution_table,
