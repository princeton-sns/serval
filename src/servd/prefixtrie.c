...
static inline void destroy_iter_node_list(struct prefix_trie_iter_node* inode) {
...
static inline struct prefix_trie_iter_node* create_iter_node(uint16_t total_bits,
        struct prefix_trie_data** branches, uint8_t branch_index, uint8_t level,
        struct prefix_trie_node* node, struct prefix_trie_iter_node* prev) {
    /*TODO - slab allocation?*/
    struct prefix_trie_iter_node* inode = (struct prefix_trie_iter_node*) malloc(sizeof(*inode));
...
static inline struct prefix_trie_data* last_prefix_of(struct prefix_trie_data* node,
        uint16_t minlen) {
    assert(node);
...
static inline uint16_t get_branch_index(struct prefix_trie_data* leaf, uint8_t offset, uint8_t bits) {
    assert(leaf);

    if(trie_get_type(leaf) != PREFIX_TRIE_TYPE_DATA || bits <= leaf->len - offset) {
        return extract_bit_value(offset, bits, leaf->key);
    }
    return extract_bit_value(offset, leaf->len - offset, leaf->key) << (bits - leaf->len + offset);

}

static inline void set_branch_child(struct prefix_trie_struct* trie,
        struct prefix_trie_node* parent, struct prefix_trie_data** branches, int branch_index,
        struct prefix_trie_data* node) {
    assert(branches);

    int filled = branches[branch_index] != NULL;

    if(branches[branch_index] && trie_get_type(branches[branch_index]) == PREFIX_TRIE_TYPE_DATA) {
...
static inline int set_node_prefix(struct prefix_trie_data* node, struct prefix_trie_data* prefix) {
    if(node == NULL) {
        return FALSE;
    }
...
static inline void destroy_trie_node(struct prefix_trie_struct* trie, struct prefix_trie_data* leaf) {
    assert(trie);
    assert(leaf);
...
static inline struct prefix_trie_data* create_trie_data(struct prefix_trie_struct* trie,
        uint8_t* key, uint16_t len, void*data, struct prefix_trie_data* prefix) {
    struct prefix_trie_data* trie_data = (struct prefix_trie_data*) malloc(sizeof(*trie_data));
...
static inline struct prefix_trie_node* create_trie_node(struct prefix_trie_struct* trie,
        uint8_t branch, uint16_t skip, uint8_t* key, struct prefix_trie_data* prefix) {

    int size = sizeof(struct prefix_trie_node) + (1 << branch) * sizeof(struct prefix_trie_data*);
    ...
    trie->node_count++;
    assert(trie_get_type((struct prefix_trie_data*) node) != PREFIX_TRIE_TYPE_DATA);
    return node;
}
...
static inline struct prefix_trie_data* split_node(struct prefix_trie_struct* trie,
        struct prefix_trie_node* node, int left) {
    assert(node);
    ...
    struct prefix_trie_node* newnode = create_trie_node(trie, bits, 0, NULL, node->prefix);
...
static inline struct prefix_trie_node* split_path(struct prefix_trie_struct* trie,
        uint8_t total_bits, uint8_t common_prefix, struct prefix_trie_data* next, uint8_t* key,
    ...
    struct prefix_trie_data* dprefix = (next->prefix && next->prefix->len <= total_bits
            + common_prefix ? next->prefix : NULL);

    struct prefix_trie_data* newleaf = create_trie_data(trie, key, prefix, data, dprefix);
    struct prefix_trie_node* newnode = create_trie_node(trie, bits, common_prefix, key, dprefix);
    ...
    if(bits == 1) {
         ...
         */
        if(nodeA) {
            set_branch_child(trie, newnode, newnode->branches, get_branch_index(nodeA, total_bits
                    + common_prefix, bits), nodeA);
        ...
        if(nodeB) {
            set_branch_child(trie, newnode, newnode->branches, get_branch_index(nodeB, total_bits
                    + common_prefix, bits), nodeB);
    ...
    assert(trie_get_type((struct prefix_trie_data*) newnode) != PREFIX_TRIE_TYPE_DATA);
    return newnode;
}

static inline int should_double(struct prefix_trie_struct* trie, struct prefix_trie_node* node) {
    assert(trie);
    if(node == NULL) {
        return FALSE;
    }
    uint8_t bits = get_branch_bits(node);
    return bits < PREFIX_TRIE_MAX_BIT && (get_branch_fill(node) + get_branch_full(node))
            / (float) (1 << (bits + 1)) >= trie->fill_factor;
}

static inline struct prefix_trie_node* double_node(struct prefix_trie_struct* trie,
        struct prefix_trie_node* node, uint8_t offset) {
    assert(trie && node);
    /*note that doubling does not change the node's full state*/
    uint8_t bits = get_branch_bits(node) + 1;

    struct prefix_trie_node* newnode = create_trie_node(trie, bits, node->skip, node->key,
            node->prefix);
    ...
    for(; i < 1 << (bits - 1); i++) {
        ...
        if(branch) {
            ...
            if(trie_get_type(branch) == PREFIX_TRIE_TYPE_DATA) {
                ...
                set_branch_child(trie, newnode, newnode->branches, newbranch, branch);
            } else {
                ...
                if(bnode->skip > 0) {
                    ...
                    if(spbranch) {
                        ...
                        set_branch_child(trie, newnode, newnode->branches, newbranch, spbranch);
...
static inline int should_halve(struct prefix_trie_struct* trie, struct prefix_trie_node* node) {
    assert(trie);
    if(node == NULL) {
        return FALSE;
    }
    uint8_t bits = get_branch_bits(node);
    return bits > 1 && (get_branch_fill(node) / (float) (1 << bits) < trie->fill_factor / 2);
}
...
static inline struct prefix_trie_data* halve_node(struct prefix_trie_struct* trie,
    ...
    if(fill > 2 || (shift_ind = check_shifted(node)) < 0) {
        ...
        newnode = create_trie_node(trie, bits - 1, node->skip, node->key, node->prefix);
        ...
        for(; i < 1 << bits; i += 2) {
            if(node->branches[i]) {
                if(node->branches[i + 1]) {
                    ...
                    hnode = create_trie_node(trie, 1, 0, node->branches[i]->key, NULL);
                    set_branch_child(trie, hnode, hnode->branches, 0, node->branches[i]);

                    if(is_node_full(node->branches[i])) {
                        inc_branch_full(hnode);
                    }
                    set_branch_child(trie, hnode, hnode->branches, 1, node->branches[i + 1]);
                    if(is_node_full(node->branches[i + 1])) {
                        inc_branch_full(hnode);
                    }

                    set_branch_child(trie, newnode, newnode->branches, i >> 1,
                            (struct prefix_trie_data*) hnode);

                    inc_branch_full(newnode);

                } else {
                    if(trie_get_type(node->branches[i]) != PREFIX_TRIE_TYPE_DATA) {
                        /*node skip increments by one*/
                        ((struct prefix_trie_node*) node->branches[i])->skip++;
                    }
                    set_branch_child(trie, newnode, newnode->branches, i >> 1, node->branches[i]);
                }
            } else if(node->branches[i + 1]) {
                if(trie_get_type(node->branches[i+1]) != PREFIX_TRIE_TYPE_DATA) {
                    /*node skip increments by one*/
                    ((struct prefix_trie_node*) node->branches[i + 1])->skip++;
                }
                set_branch_child(trie, newnode, newnode->branches, i >> 1, node->branches[i + 1]);
            }
        }
    } else {
        /*fill == 2 and all the nodes in a single bit bucket*/
        /*TODO may also generate a doubled node*/
        newnode = create_trie_node(trie, 1, node->skip + bits - 1, NULL, node->prefix);
        set_branch_child(trie, newnode, newnode->branches, 0, node->branches[shift_ind]);
        if(is_node_full(node->branches[shift_ind])) {
            inc_branch_full(newnode);
        }

        set_branch_child(trie, newnode, newnode->branches, 1, node->branches[shift_ind + 1]);
...
static inline void update_shortest(struct prefix_trie_struct* trie,
    ...
    while(inode) {
        for(i = 0; i < inode->limit; i++) {
            if(inode->branches[i] && inode->branches[i]->shortest < shortest) {
...
static inline void* search_prefix(struct prefix_trie_data* prefix, uint8_t* key, uint16_t klen,
        uint8_t total_bits, uint8_t* found) {
    while(prefix) {
        //printf("comparing klen: %u total bits: %u to prefix: %s\n", klen, total_bits, print_trie_data(prefix));
        if(klen >= prefix->len && is_bitstring_equal(key, prefix->key, total_bits, prefix->len
                - total_bits)) {
            if(found) {
                *found = TRUE;
            }
...
static inline void* search_prefix_exact(struct prefix_trie_data* prefix, uint8_t* key,
        uint16_t klen, uint8_t total_bits, uint8_t* found) {
    while(prefix) {
        if(klen == prefix->len && is_bitstring_equal(key, prefix->key, total_bits, prefix->len
                - total_bits)) {
            if(found) {
...
static inline struct prefix_trie_data* find_singleton_child(struct prefix_trie_node* node) {
    assert(node);
...
static void update_shortest(struct prefix_trie_struct* trie, struct prefix_trie_iter_node* inode);

static void* insert_prefix(struct prefix_trie_struct* trie, struct prefix_trie_data* leaf,
        struct prefix_trie_data* prefix, int freenew);
static void* remove_prefix(struct prefix_trie_struct* trie, struct prefix_trie_data* dnode,
        uint8_t* key, uint16_t prefix, struct prefix_trie_data** oleaf, short *removed);

static void* remove_interstitial(struct prefix_trie_struct* trie, uint8_t offset,
        uint8_t level_bits, uint8_t* key, uint16_t prefix, struct prefix_trie_data** branches,
        struct prefix_trie_node* node, short* remd);
static void* insert_interstitial(struct prefix_trie_struct* trie, uint8_t offset,
        uint8_t level_bits, uint8_t* key, uint16_t prefix, void* data,
        struct prefix_trie_data** branches, struct prefix_trie_node* node, uint8_t* is_existing);

static void* search_alternates(struct prefix_trie_struct* trie, struct prefix_trie_node* node,
        struct prefix_trie_data* last_prefix, uint8_t* key, uint16_t prefix, uint8_t total_bits,
        uint8_t level_bits, struct prefix_trie_data** branches, uint8_t branch_index);

static void _prefix_trie_iter_init(struct prefix_trie_iter* iter, struct prefix_trie_struct* trie,
        int print_out);
static int _prefix_trie_iter_next(struct prefix_trie_iter* iter, uint8_t** key, uint16_t* prefix,
...
static void* insert_interstitial(struct prefix_trie_struct* trie, uint8_t offset,
    ...
    if(level_bits <= prefix - offset) {
        LOG_ERR("Replicated entries must fall within the level bits! %u >= %u", prefix, level_bits);
        return NULL;
    }
    //char buffer[128];
    /*replicate it*/
    struct prefix_trie_data* newleaf = create_trie_data(trie, key, prefix, data, NULL);

    uint16_t branch_index = extract_bit_value(offset, prefix - offset, key) << (level_bits - prefix
            + offset);
...
static void* remove_prefix(struct prefix_trie_struct* trie, struct prefix_trie_data* dnode,
    ...
    int nshortest = trie_get_type(dnode) == PREFIX_TRIE_TYPE_DATA ? dnode->len : 0xFF;
            ...
            *removed = TRUE;
            ...
            if(dnode->shortest == prefix) {
                ...
                if(trie_get_type(orig) != PREFIX_TRIE_TYPE_DATA) {
                    int i;
                    struct prefix_trie_node* node = (struct prefix_trie_node*) orig;
                    for(i = 0; i < 1 << get_branch_bits(node); i++) {
                        if(node->branches[i] && node->branches[i]->shortest < node->shortest) {
...
void* prefix_trie_insert(struct prefix_trie_struct* trie, uint8_t* key, uint16_t prefix, void* data) {
    assert(trie);
    ...
    while(bits_left >= level_bits) {
        ...
        if(next) {
            ...
            common_prefix = find_longest_common_prefix(key, next->key, total_bits, (prefix
                    > node_prefix ? node_prefix - total_bits : bits_left));
            ...
            if(prev && common_prefix == 0) {
                ...
                if(should_double(trie, prev)) {
                    ...
                    set_branch_child(trie, prev2, prev_branches, prev_branch_index,
                            (struct prefix_trie_data*) node);
                    ...
                    branch_index = get_branch_index(newleaf, total_bits - level_bits,
                    ...
                    set_branch_child(trie, node, node->branches, branch_index, newleaf);
            ...
            node = split_path(trie, total_bits, common_prefix, next, key, prefix, data);
            ...
            assert(trie_get_type((struct prefix_trie_data* ) node) != PREFIX_TRIE_TYPE_DATA);
            set_branch_child(trie, prev, branches, branch_index, (struct prefix_trie_data*) node);
            ...
            if(prev) {
                if(should_double(trie, prev)) {
                    ...
                    set_branch_child(trie, prev2, prev_branches, prev_branch_index,
                            (struct prefix_trie_data*) node);
                    destroy_trie_node(trie, (struct prefix_trie_data*) prev);
                } else if(prev2 && is_node_full((struct prefix_trie_data*) prev) && !node_full) {
                    inc_branch_full(prev2);

                    if(should_double(trie, prev2)) {
                        print_trie_node(prev2, total_bits - level_bits - prev->skip
                                - get_branch_bits(prev2));

                        node = double_node(trie, prev2, total_bits - level_bits - prev->skip
                                - get_branch_bits(prev2));
                        set_branch_child(trie, prev3, prev2_branches, prev2_branch_index,
                                (struct prefix_trie_data*) node);
                        destroy_trie_node(trie, (struct prefix_trie_data*) prev2);
                    }
                }
            }
            goto out;
    ...
    odata = insert_interstitial(trie, total_bits, level_bits, key, prefix, data, branches, node,
            &is_data);
    ...
    if(!is_data && node) {
        if(should_double(trie, node)) {
            ...
            destroy_trie_node(trie, (struct prefix_trie_data*) node);
        } else if(prev && is_node_full((struct prefix_trie_data*) node) && !node_full) {
            inc_branch_full(prev);

            if(should_double(trie, prev)) {
                print_trie_node(prev, total_bits - node->skip - get_branch_bits(prev));

                prev3 = double_node(trie, prev, total_bits - node->skip - get_branch_bits(prev));

                set_branch_child(trie, prev2, prev2_branches, prev_branch_index,
...
int prefix_trie_initialize(struct prefix_trie_struct* trie, size_t keylen, float fillfactor) {
    assert(trie);
...
void* prefix_trie_remove(struct prefix_trie_struct* trie, uint8_t* key, uint16_t prefix) {
    ...
    if(prefix == 0) {
        ...
        if(odata) {
            ...
            for(i = 0; i < PREFIX_TRIE_ROOT_BRANCH; i++) {
                if(trie->root_branch[i] && trie->root_branch[i]->shortest < shortest) {
    ...
    while(bits_left >= level_bits) {
        if(prefix == shortest) {
            ...
            track = create_iter_node(total_bits, branches, branch_index, 0, node, track);
        ...
        if(next) {
            ...
            if(prefix == node_prefix && is_data) {
                ...
                if(is_bitstring_equal(key, next->key, total_bits, bits_left)) {
                    ...
                    if(set_node_prefix(next, NULL)) {
                        ...
                        set_branch_child(trie, prev, branches, branch_index, temp);
                        assert(temp->ref_count == 1);
                    } else {
                        is_full = is_node_full((struct prefix_trie_data*) prev);
                        set_branch_child(trie, prev, branches, branch_index, NULL);
                        ...
                        if(prev) {
                            ...
                            if(get_branch_fill(prev) == 1) {
                                ...
                                for(i = 0; i < 1 << get_branch_bits(prev); i++) {
                                    if(prev->branches[i]) {
                                        printf(
                                                ...
                                                prev_branch_index, print_trie_node(prev, total_bits
                                                        - level_bits), print_trie_data(
                                                        prev->branches[i]));
                                        if(prev2) {
                                            printf("grandparent before: %s\n", print_trie_node(
                                                    prev2, total_bits - level_bits - prev->skip
                                                            - get_branch_bits(prev2)));
                                        }

                                        set_branch_child(trie, prev2, prev_branches,
                                                prev_branch_index, prev->branches[i]);

                                        if(prev2) {
                                            printf("grandparent after: %s\n", print_trie_node(
                                                    prev2, total_bits - level_bits - prev->skip
                                                            - get_branch_bits(prev2)));
                                        }
                                        ...
                                        if(prev->prefix) {
                                            ...
                                            insert_prefix(trie, prev->branches[i], prev->prefix,
                                                    FALSE);
                                        }
                                        ...
                                        if(trie_get_type(prev->branches[i])
                                            ...
                                            ((struct prefix_trie_node*) prev->branches[i])->skip
                                                    += get_branch_bits(prev) + prev->skip;
                                        }
                                        break;
                                    }
                                }
                                ...
                                if(prev2 && is_full) {
                                    ...
                                    printf("singleton leaf replace full change prev2: %s\n", print_trie_node(prev2,total_bits - level_bits - prev->skip- get_branch_bits(prev2)));
                                }
                                ...
                                if(prev2) {
                                    ...
                                    printf("halved node possible full change prev2: %s\n", print_trie_node(prev2,total_bits - level_bits - prev->skip- get_branch_bits(prev2)));
                                }
                            } else if(prev2 && is_full && !is_node_full(
                                    (struct prefix_trie_data*) prev)) {
                                /*full status of prev changed - update its parent*/
                                assert(get_branch_full(prev2) > 0);
                                dec_branch_full(prev2);
                                printf("regular remove full change prev2: %s\n", print_trie_node(prev2,total_bits - level_bits - prev->skip- get_branch_bits(prev2)));
                            }
                        }
                    }
            ...
            if(node_prefix >= prefix) {
                ...
                common_prefix = find_longest_common_prefix(key, next->key, 0, node_prefix);
            ...
            if(is_bitstring_equal(key, next->key, total_bits, node_prefix - total_bits)) {
                //printf("continuing on the node path\n");
                total_bits += node->skip;
                bits_left -= node->skip;
                /*continue*/
    ...
    odata
            = remove_interstitial(trie, total_bits, level_bits, key, prefix, branches, node,
                    &removed);
    ...
    if(removed) {
        ...
        if(node) {
            if(get_branch_fill(node) == 1) {
                ...
                for(i = 0; i < 1 << get_branch_bits(node); i++) {
                    if(node->branches[i]) {
                        set_branch_child(trie, prev, prev_branches, branch_index, node->branches[i]);

                        /*if the node had a prefix, then the child must inherit it*/
                        if(node->prefix) {
                            //set_node_prefix(last_prefix_of(node->branches[i], 0), node->prefix);
                            insert_prefix(trie, node->branches[i], node->prefix, FALSE);
                        }
                ...
                if(prev && is_full) {
                    ...
                    printf("interstitial singleton replace full change prev: %s\n", print_trie_node(prev,total_bits));
                }
            ...
            } else if(prev && is_full && !is_node_full((struct prefix_trie_data*) node)) {
                /*full status of prev changed - update its parent*/
                assert(get_branch_full(prev) > 0);
                dec_branch_full(prev);
...
static void* search_alternates(struct prefix_trie_struct* trie, struct prefix_trie_node* node,
     ...
     *then search.
     */
    if(branches[branch_index] && branches[branch_index]->shortest <= prefix) {
        //printf("searching prefix tree!\n");
        data = search_prefix(branches[branch_index]->prefix, key, prefix, total_bits, &found);
    }
    ...
    if(!found) {
        ...
        if(level_bits > 1 && ((node && node->shortest < total_bits + level_bits) || (!node
                && trie->shortest < PREFIX_TRIE_ROOT_BIT))) {
            ...
            for(bit_check = 1; bit_check < level_bits; bit_check++) {
                ...
                if(next) {
                    ...
                    if(trie_get_type(next) == PREFIX_TRIE_TYPE_DATA) {
                        if(total_bits + (level_bits - bit_check) >= next->len && next->len
                                <= prefix) {
                            return next->data;
                        }
                        if(total_bits + (level_bits - bit_check) >= next->shortest
                                && next->shortest <= prefix) {
                            data = search_prefix(next->prefix, key, prefix, total_bits, &found);
                            assert(found);
                            break;
                        }
    ...
    if(!found && last_prefix && last_prefix->shortest <= prefix) {
        ...
        data = search_prefix(last_prefix, key, prefix, last_prefix->shortest, &found);
    ...
    while(bits_left >= level_bits) {
        ...
        if(next) {
            ...
            if(trie_get_type(next) == PREFIX_TRIE_TYPE_DATA) {
                ...
                if(prefix >= next->len && is_bitstring_equal(key, next->key, total_bits, (prefix
                        > next->len ? next->len - total_bits : prefix - total_bits)) == TRUE) {
                ...
                data = search_alternates(trie, prev, last_prefix, key, prefix, total_bits
                        - level_bits, level_bits, branches, branch_index);
            ...
            if(bits_left <= node->skip || prefix < node->shortest || !is_bitstring_equal(key,
                    next->key, total_bits, (bits_left > node->skip ? node->skip : bits_left))) {
            ...
            return search_alternates(trie, prev, last_prefix, key, prefix, total_bits - level_bits,
                    level_bits, branches, branch_index);
        }
    }

    branch_index = extract_bit_value(total_bits, bits_left, key) << (level_bits - bits_left);
    //printf("we're somewhere in the middle - find the right branch value: %i\n", branch_index);

    return search_alternates(trie, node, last_prefix, key, prefix, total_bits, level_bits,
    ...
    while(bits_left >= level_bits) {
        ...
        if(next) {
            ...
            if(trie_get_type(next) == PREFIX_TRIE_TYPE_DATA) {
                ...
                if(prefix == next->len && is_bitstring_equal(key, next->key, total_bits, (prefix
                        > next->len ? next->len - total_bits : bits_left)) == TRUE) {
                    return next->data;
                }
                /*search the prefix tree */
                //printf("search the data prefix tree\n");
                return search_prefix_exact(next->prefix, key, prefix, total_bits, &found);
            ...
            if(prefix - total_bits <= node->skip || !is_bitstring_equal(key, next->key, total_bits,
                    (prefix - total_bits > node->skip ? node->skip : bits_left))) {
                //printf("searching prefix: bits left %u node skip: %u total bits: %u\n", bits_left,node->skip, total_bits);
                return search_prefix_exact(next->prefix, key, prefix, total_bits, &found);
            }
    ...
    while(prefix - total_bits >= level_bits) {
        ...
        if(next) {
            ...
            if(trie_get_type(next) == PREFIX_TRIE_TYPE_DATA) {
                ...
                /*search the prefix tree */
                //printf("search the data prefix tree\n");
                search_prefix_exact(next->prefix, key, prefix, total_bits, &found);
                return found;
            }
...
static void _prefix_trie_iter_init(struct prefix_trie_iter* iter, struct prefix_trie_struct* trie,
    ...
    /*root iter node*/
    struct prefix_trie_iter_node* inode = create_iter_node(0, trie->root_branch, 0, 0, NULL, NULL);
...
static int _prefix_trie_iter_next(struct prefix_trie_iter* iter, uint8_t** key, uint16_t* prefix,
    ...
    if(inode->prefix) {
        ...
        if(print_out) {
            memset(buffer, ' ', inode->level);
            buffer[inode->level] = 0;
            printf("%sdata(%u): %s\n", buffer, inode->branch, print_trie_data(
                    inode->branches[inode->branch]));
        }

        if(inode->branches[inode->branch]->prefix && inode->branches[inode->branch]->prefix->len
                > inode->len) {
    ...
    int level_bits = (inode->node == NULL ? PREFIX_TRIE_ROOT_BIT : get_branch_bits(inode->node));
...
static inline struct prefix_trie_iter_node* get_iter_node_at_level(uint16_t level,
        struct prefix_trie_iter_node* inode) {
    assert(inode);
...
int prefix_trie_iter_remove(struct prefix_trie_iter* iter) {
    ...
    if(inode->prefix) {
        ...
        set_node_prefix(last_prefix_of(inode->branches[inode->branch], inode->prefix->len),
                inode->prefix->prefix);

        if((inode->node && inode->node->shortest == inode->prefix->len) || (iter->trie->shortest
                == inode->prefix->len)) {
    ...
    if(set_node_prefix(leaf, NULL)) {
        ...
        set_branch_child(iter->trie, inode->node, inode->branches, inode->branch, next);
        ...
        set_branch_child(iter->trie, inode->node, inode->branches, inode->branch, NULL);
    }

    if((inode->node && inode->node->shortest == leaf->len) || (iter->trie->shortest == leaf->len)) {
        update_shortest(iter->trie, inode);
    }
    ...
    if(inode->node) {
        ...
        if(get_branch_fill(inode->node) == 1 && inode->prev) {
            ...
            prevnode = get_iter_node_at_level(inode->level + 1, iter->iter_node);
            if(prevnode) {
            ...
            set_branch_child(iter->trie, inode->prev->node, inode->prev->branches, branch, next);
            ...
            if(trie_get_type(next) != PREFIX_TRIE_TYPE_DATA) {
                ((struct prefix_trie_node*) next)->skip += get_branch_bits(inode->node)
                        + inode->node->skip;
            }

            destroy_trie_node(iter->trie, (struct prefix_trie_data*) inode->node);
            goto out;
            ...
            if(inode->node == newnode->node) {
                ...
                if(trie_get_type(next) == PREFIX_TRIE_TYPE_DATA
                    ...
                    tempnode = create_iter_node(newnode->len + get_branch_bits(newnode->node) - 1,
                            node->branches, newnode->branch % 2, newnode->level + 1, node, newnode);
