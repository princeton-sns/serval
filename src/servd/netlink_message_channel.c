...
static const struct sockaddr *netlink_get_local_address(message_channel *
							channel, int *len);
static void netlink_set_peer_address(message_channel * channel,
				     struct sockaddr *addr, size_t len);
static const struct sockaddr *netlink_get_peer_address(message_channel *
						       channel, int *len);
static int netlink_send(message_channel * channel, void *message,
			size_t datalen);
static int netlink_send_iov(message_channel * channel, struct iovec *iov,
			    size_t veclen, size_t datalen);

static int netlink_recv(message_channel * channel, const void *message,
			size_t datalen);
...
static int _netlink_send(struct sv_netlink_message_channel *nchannel,
			 struct iovec *iov, size_t veclen);

static inline void skip_seq_num(struct sv_netlink_message_channel
				*nchannel, uint32_t seq_num)
{
...
static inline int is_acked(struct sv_netlink_message_channel *nchannel,
			   uint32_t seq_num)
{
...
static void ack_seq_num(struct sv_netlink_message_channel *nchannel,
			uint32_t seq_num)
{
...
message_channel *create_netlink_message_channel(int protocol,
						int buffer_len,
						int reliable,
						message_channel_callback *
						callback)
{
    struct sv_netlink_message_channel *nchannel =
	(struct sv_netlink_message_channel *)
	malloc(sizeof(struct sv_netlink_message_channel));
				     ...
				     struct sockaddr *addr, size_t len)
{
    ...
    struct sv_netlink_message_channel *nchannel =
	(struct sv_netlink_message_channel *) channel;
...
const struct sockaddr *netlink_get_peer_address(message_channel * channel,
						int *len)
{
    assert(channel);
...
static int netlink_initialize(message_channel * channel)
{
    ...
    ret =
	bind(nchannel->base.sock, (struct sockaddr *) &nchannel->peer,
	     sizeof(nchannel->peer));
			 ...
			 struct iovec *iov, size_t veclen)
{
    ...
    struct msghdr mh =
	{ &nchannel->peer, sizeof(nchannel->peer), iov, veclen, NULL, 0,
	0
    };
    ...
    while (atomic_read(&nchannel->base.running) && ret < 0
			...
			size_t datalen)
{
    ...
    LOG_DBG("Received NETLINK %zu byte message from the local stack\n",
	    datalen);
    for (nlm = (struct nlmsghdr *) message;
	...
	case NLMSG_SERVAL:
	    ...
	    nchannel->base.callback.recv_message(&nchannel->base.callback,
						 NLMSG_DATA(nlm),
...
static void netlink_recv_task(void *channel)
{
	...
	//MSG_DONTWAIT
	ret =
	    recvfrom(nchannel->base.sock, nchannel->base.buffer,
		     nchannel->base.buffer_len, 0,
	...
	nchannel->base.channel.interface->recv_message(&nchannel->base.channel,
						       nchannel->base.buffer,
						       ret);
    }
