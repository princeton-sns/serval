...
static int execute_task(struct poll_reactor_ds *reactor,
			struct fdtask *task, int flags);
static int execute_timeout(struct poll_reactor_ds *reactor, long long timeout);
static int generate_epoll_ctl_flags(struct poll_reactor_ds *reactor,
				    struct fdtask *task);
static int register_interest(struct poll_reactor_ds *reactor, int fd,
			     struct fdtask *task, int op);
...
void pr_add_timeout(struct poll_reactor_ds *reactor, time_t msec,
		    reactor_exec exec, void *target)
{
    ...
    GSequenceIter *iter =
	g_sequence_search(reactor->timeout_seq, &le, timeout_comp, NULL);
	...
	iter =
	    g_sequence_insert_sorted(reactor->timeout_seq, entry,
				     timeout_comp, NULL);
...
int pr_cancel_timeout(struct poll_reactor_ds *reactor, reactor_exec exec,
		      void *target)
{
    assert(reactor);
    if (exec == NULL) {
...
int pr_is_waiting(struct poll_reactor_ds *reactor, reactor_exec exec,
		  void *target)
{
    assert(reactor);
    if (exec == NULL) {
...
void pr_set_read(struct poll_reactor_ds *reactor, int fd,
		 reactor_exec exec, void *target)
{
    assert(exec);
...
void pr_set_write(struct poll_reactor_ds *reactor, int fd,
		  reactor_exec exec, void *target)
{
    assert(reactor);
...
void pr_set_error(struct poll_reactor_ds *reactor, int fd,
		  reactor_exec exec, void *target)
{
    assert(reactor);
...
void pr_set_interest(struct poll_reactor_ds *reactor, int fd,
		     reactor_exec rexec, void *rdata, reactor_exec wexec,
		     void *wdata, reactor_exec eexec, void *edata)
{
    assert(reactor);
...
void pr_start(struct poll_reactor_ds *reactor)
{
	...
	LOG_DBG
	    ("polling with timeout: %lli, read interest: %i, write interest: %i, max events: %i\n",
	     delta, reactor->read_count, reactor->write_count,
	     reactor->max_events);

	/* TODO might need to differentiate between poll events and lingering "ready" state
	 * for active fd's to avoid starvation*/
	num_fds =
	    epoll_wait(reactor->poll_fd, events, reactor->max_events,
		       (int) delta);
...
{
    struct poll_reactor_ds *reactor =
	(struct poll_reactor_ds *) malloc(sizeof(*reactor));
			...
			struct fdtask *task, int flags)
{
	...
	if (!
	    (task->read_task.exec == task->write_task.exec
	     && task->read_task.target == task->write_task.target
	     && flags & EPOLLIN)) {
	...
	if (!
	    (task->read_task.exec == task->error_task.exec
	     && task->read_task.target == task->error_task.target
	     && flags & EPOLLIN)
|| !(task->write_task.exec == task->error_task.exec
     && task->write_task.target == task->error_task.target
     && flags & EPOLLOUT)) {
