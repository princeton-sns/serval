...
typedef struct sv_resolver_message_callback {
    void* target;
    void
    (*resolver_message_cb)(struct sv_resolver_message_callback* cb, uint16_t type, struct sv_control_header* message, size_t len,
...
struct sv_resolver_rpc_interface {
    ...
    void (*set_local_address)(resolver_rpc* messenger, struct sockaddr* saddr, size_t len);

    void (*set_callback) (resolver_rpc* messenger, resolver_message_callback* callback);
    ...
    /* resolver rpc */
    int (*send_resolver_message)(resolver_rpc* messenger, uint32_t xid, struct sv_control_header* message,
            size_t len, resolver_message_callback* rm_cb);
};


void resolver_rpc_set_peer(resolver_rpc* messenger, struct sv_instance_addr* peer_addr);
