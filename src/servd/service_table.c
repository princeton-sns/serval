...
static inline struct ref_entry *find_reference_in_list(struct ref_list
                                                       *rlist, struct service_id
                                                       *sid, struct net_addr
                                                       *address)
{
    struct ref_entry *entry;
    list_for_each_entry(entry, &rlist->head, head) {
        if (memcmp(entry->ref->instance.service.sv_srvid.srv_un.un_id8,
                   sid->srv_un.un_id8, 32) == 0
            && memcmp(&entry->ref->instance.address.sin.sin_addr,
                      &address->net_un.un_ip, sizeof(struct in_addr)) == 0) {
            return entry;
        }
    }
    return NULL;
...
{
    assert(st);
    return prefix_trie_initialize(&st->service_trie, SERVICE_KEY_LEN,
                                  SERVICE_FILL_FACTOR);
}
...
void service_table_iter_init(struct service_table_iter *iter,
                             struct sv_service_table *st)
{
...
int service_table_iter_next(struct service_table_iter *iter,
                            struct service_reference **sref)
{
    ...
    if (iter->pri_array) {
        if (iter->cur_index < iter->pri_array->len - 1) {
            iter->cur_list
                =
                (struct ref_list *) g_ptr_array_index(iter->pri_array,
                                                      ++iter->cur_index);
            assert(iter->cur_list);
            assert(!list_empty(&iter->cur_list->head));
    ...
    if (prefix_trie_iter_next
        (&iter->trie_iter, NULL, NULL, (void **) &iter->pri_array)) {
        assert(iter->pri_array);
        assert(iter->pri_array->len > 0);
        iter->cur_list =
            (struct ref_list *) g_ptr_array_index(iter->pri_array,
...
size_t service_table_iter_reference_count(struct service_table_iter *iter)
{
    ...
    if (iter->pri_array) {
        ...
        for (; i < iter->pri_array->len; i++) {
            count += ((struct ref_list *)
                      g_ptr_array_index(iter->pri_array, i))->count;
        }
    }
    return count;
}
 ...
 */
void service_table_iter_remove(struct service_table_iter *iter,
                               struct service_reference **sref)
{
    assert(iter);
...
int service_table_add_service_reference(struct sv_service_table *st,
                                        struct service_reference *sref)
{
    assert(st);
    assert(sref);
    ...
    GPtrArray *pri_array = (GPtrArray *) prefix_trie_find(&st->service_trie,
                                                          ...
                                                          un_id8,
                                                          sref->instance.
                                                          service.sv_prefix_bits);

    if (pri_array == NULL) {
        pri_array = g_ptr_array_new();
        prefix_trie_insert(&st->service_trie,
                           sref->instance.service.sv_srvid.srv_un.un_id8,
    ...
    if (find_reference_in_list(rlist, &sref->instance.service.sv_srvid,
                               (struct net_addr *) &sref->instance.address.sin.
                               sin_addr) != NULL) {
...
int service_table_remove_service_reference(struct sv_service_table *st,
                                           uint8_t flags, uint8_t prefix,
                                           struct service_id *sid,
...
struct service_reference *service_table_find_service_reference(struct
                                                               sv_service_table
                                                               *st,
                                                               uint8_t
                                                               flags,
...
int service_table_find_service_references(struct sv_service_table *st,
                                          uint8_t flags, uint8_t prefix,
                                          struct service_id *sid,
                                          struct service_reference ***sref,
                                          size_t * cnt)
{
    ...
    struct service_reference **srefs =
        (struct service_reference **) malloc(sizeof(*srefs) * count);
...
struct service_reference *service_table_resolve_service(struct
                                                        sv_service_table
                                                        *st, uint8_t flags,
                                                        uint8_t prefix,
                                                        struct service_id
...
int service_table_update_service_stats(struct sv_service_table *st,
                                       struct sv_service_stats *sstat)
{
    return 0;
}

int service_table_get_table_stats(struct sv_service_table *st,
                                  struct sv_table_stats *tstat)
{
    /*not implemented yet */
    return 0;
}
...
void print_service_table(FILE * fptr, struct sv_service_table *table)
{
    ...
    while (service_table_iter_next(&iter, &ref)) {
        bytes =
            ref->instance.service.sv_prefix_bits / 8 +
            (ref->instance.service.sv_prefix_bits % 8 == 0 ? 0 : 1);
        for (i = 0; i < bytes; i++) {
            len += snprintf(&buffer[i * 2], 128 - len, "%02x",
                            ref->instance.service.sv_srvid.
                            srv_un.un_id8[i] & 0xff);

        }

        fprintf(fptr, "%-64s %-6i %-18s %-10i %-10i %-10i\n", buffer,
                ref->instance.service.sv_prefix_bits,
                inet_ntoa(ref->instance.address.sin.sin_addr),
                ref->priority, ref->weight, ref->ttl);
    }
