...
typedef struct sv_path_async_callback {
    void* target;
    void (*resolution_path_cb)(struct sv_path_async_callback* target, uint32_t xid, uint16_t status, void* data);
...
struct sv_resolution_path_interface {
    ...
    void (*set_path_callback)(resolution_path* target, resolution_path_callback* resolver);
    const resolution_path_callback* (*get_path_callback)(resolution_path* target);

    int (*configure_interface)(resolution_path* path, const char *ifname,
            const struct net_addr *ipaddr, unsigned short flags);

    int (*get_service_stats)(resolution_path* path, struct service_stat* stats);
