...
inline uint8_t extract_bit_value(uint8_t pos, uint8_t len, uint8_t * key)
{
	...
	| (shift_bits + len >
	   8 ? (key[pos + 1] >> (16 - shift_bits - len)) : 0);
}
		       ...
		       uint16_t len)
{
    ...
    if (shift_bits + len > 8
	&& (shift_bits == 0 || (uint8_t) (strA[offset] << shift_bits)
	    >> shift_bits ==
	    (uint8_t) (strB[offset] << shift_bits) >> shift_bits)) {
	if (shift_bits > 0) {
	    offset++;
	}
	...
	//                + offset, byte_len));
	if (byte_len == 0
	    || (memcmp(strA + offset, strB + offset, byte_len) == 0)) {
	    /*no trailing bits */
	    if (shift_bits == 8) {
		return TRUE;
	    }

	    return strA[offset + byte_len] >> shift_bits ==
		strB[offset + byte_len] >> shift_bits;
	}
    } else {
	/*check each bit - first byte match failed */
	//printf("first byte check failed, check bits\n");
	int val = check_byte_bits(strA[offset] << shift_bits,
				  strB[offset] << shift_bits,
				  (shift_bits + len >
				   8 ? 8 - shift_bits : len));
				    ...
				    uint16_t offset, uint16_t len)
{
	...
	if (shift_bits > 0) {
		...
		prefix +=
		    check_byte_bits(strA[byte_end], strB[byte_end],
				    (8 - shift_bits));
	    }
	}
    } else {
	...
	//printf("first byte check failed, check bits\n");
	prefix =
	    check_byte_bits(strA[offset] << shift_bits,
			    strB[offset] << shift_bits,
				    ...
				    struct service_reference *ref)
{
    ...
    memcpy(&res->srvid, &ref->instance.service.sv_srvid,
	   sizeof(struct service_id));
    memcpy(&res->address.net_un.un_ip, &ref->instance.address.sin.sin_addr,
				     ...
				     struct service_reference *ref)
{
    ...
    memcpy(&sdesc->service, &ref->instance.service.sv_srvid,
	   sizeof(struct service_id));
}
...
{
    int option = 1;
    if (setsockopt(soc, SOL_SOCKET, SO_REUSEADDR, &option, sizeof(option))
	< 0) {
...
void initialize_service_id(struct service_id *sid, uint16_t prefix)
{
    ...
    sid->srv_un.un_id8[offset++] =
	(idb & ~mask) | ((((uint8_t *) & val)[0]) & mask);
...
inline void init_control_header(struct sv_control_header *header,
				uint8_t type, uint32_t xid, uint16_t len)
{
    assert(header);
